...
## ADR-015: Form config theo tenant qua registry + Zod meta-validator (1 file/trường)

- **Context**: `getApplicationFormSchema` cũ hardcode 3 hằng số `FormSchema` + phân nhánh `if (tenantCode === ...)` trong 1 file `lib/api/forms.ts`. Thêm trường = sửa hàm chung + thêm const → không scale, dễ va chạm và **sót field** (đã xảy ra: form Nguyễn Gia Thiều bị cụt còn 5 field sau các lần sửa). North Star (CLAUDE.md #6) là form = JSON config, đổi không cần redeploy — nhưng **chưa có validator cho chính cái config**, một schema dựng sai render ra form trống mà không báo lỗi.
- **Decision**: Tách config thành **1 file/tenant** trong `apps/web/lib/api/form-schemas/` (mỗi file export object `satisfies FormSchema`, dùng chung helper trong `_fields.ts`), gom vào registry `index.ts` (`code → schema`). Thêm **Zod meta-validator** `formSchemaSchema` + `parseFormSchema()` ở `@shared/form` (`meta-schema.ts`) — hợp đồng "form hợp lệ là gì", **khác** `build-zod.ts` (validator *câu trả lời*, không phải *định nghĩa form*). Registry validate mọi config lúc load: **dev throw** (fail loud), **prod log + dùng default** (giống fallback của `getTenantBranding`). `getApplicationFormSchema(code)` rút còn `REGISTRY[code] ?? DEFAULT` — **giữ nguyên chữ ký + 4 call site**. `parseFormSchema` trả `FormSchema` **không cast**: nếu validator lệch type, return ngừng compile (drift guard).
- **Consequences**: + Thêm trường = thả 1 file + 1 dòng map, không đụng logic chung. + Config sai bị bắt sớm (load-time) thay vì render trống. + Seam pha 1→2 sạch: ruột `getApplicationFormSchema` đổi từ registry lookup → `SELECT schema FROM tenant_form_schemas` + `parseFormSchema(row.schema)`; **cùng meta-validator** validate JSONB từ DB / admin form-builder tương lai; type + call site bất biến. − Config vẫn là file TS pha 1 (đổi form vẫn cần redeploy) — "no redeploy" chỉ đạt khi pha 2 chuyển sang DB. − Meta-validator phải đồng bộ tay với type `FormSchema` (giảm rủi ro bằng drift guard uncast + check `FORM_FIELD_TYPES` đủ nhánh lúc load). − Chưa gộp branding/applicant-config/landing vào cùng "1 file tenant" — để mở (out of scope).
